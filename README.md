...
Make sure you get your target ip address available. i,m so sure that you most have gotten ready the ip you want to target. If you dont have you can get one through metasploitable2 vm. Better still you can use your ohter ip device for save practise. Note: for this tutorial, i will use 192.168.*.*
...
The above command will import the targeted scanned rsult to metasploit database. If the above command is not connecting, no connection, that means that metasploit (postgresql) is not running. To fix it start "postgresql" by running the below command:
...
From the option below on the screenshot, choose i. run:
info 1
...
When you check yhe last output you will noticed that we have gotten a shell. That means you are in yor targeted device. You now have access to go through your target directories and escalate previledges. 
